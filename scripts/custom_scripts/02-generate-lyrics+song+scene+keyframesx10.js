 ...
 *
 * Output:
 *   scripts/custom_scripts/batches/<YYYYMMDD>/
 *     characters/          shared refs for this date
 *     scenes/              shared empty room stills
 *     <song_slug>/
 *       <song_slug>.mp3
 *       lyrics.txt
 ...
 *
 * Character defs live in scripts/custom_scripts/family/ (does NOT touch
 * generate-dataset.js or root character.json).
 *
 * Run:
 *   node scripts/custom_scripts/02-generate-lyrics+song+scene+keyframesx10.js
 *
 * Optional:
...
const FAMILY_DIR = join(SCRIPT_DIR, "family");
const ACE_ROOT =
...
async function loadFamilyCast() {
  ...
  const cast = {};
  for (const id of ids) {
    cast[id] = await loadJson(join(FAMILY_DIR, `${id}.json`));
  }
  const scenes = await loadJson(join(FAMILY_DIR, "scenes.json"));
  return { cast, scenes };
}
...
async function main() {
  ...
  const batchDir = join(SCRIPT_DIR, "batches", dateStamp());
  const sharedCharsDir = join(batchDir, "characters");
  const sharedScenesDir = join(batchDir, "scenes");
