...
export function continuityMotionExtras(beat, prevBeat) {
  ...
  if (phase === "setup") {
    parts.push("quiet setup noticing moment, soft idle, story about to begin");
    ...
    parts.push("mid-action clear readable motion driving the story");
  }
  ...
  if (enterDir === "left") parts.push("entering from the left side of frame");
  if (enterDir === "right") parts.push("entering from the right side of frame");

  if (prevBeat && roomFamily(prevBeat.location) === roomFamily(beat?.location)) {
    const prevSlot = slotOf(prevBeat);
    if (prevSlot === slot) {
      parts.push(
