...
async function comfyHostStats(comfyUrl) {
  ...
  try {
    const res = await fetch(`${comfyUrl.replace(/\/$/, "")}/system_stats`, {
      ...
      signal: AbortSignal.timeout(isSaladUrl(comfyUrl) ? 6000 : 2500),
    });
...
export async function saladSystemMetrics() {
  ...
  return {
    ...
    comfyUp: !!host?.comfyUp,
    instance,
    note: available
      ? cpuPercent == null && !saladMgmtConfigured()
        ? "CPU% needs SALAD_ORG in .env (instance API)"
        : null
      : gateway && !host
        ? "Gateway set but Comfy not reachable (start Salad container)"
        : base.note ||
          (!saladContainer()
