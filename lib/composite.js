...
export function resolveCharacterLayout({
  ...
  // Depth variety: near = foreground, far = toward furniture/sink
  if (d === "near") {
    bottomPad = Math.max(0.04, bottomPad - 0.04);
    scale = Math.min(outdoor ? 0.34 : 0.46, scale * 1.06);
  } else if (d === "far") {
    bottomPad = Math.min(0.22, bottomPad + 0.06);
    scale = Math.max(0.22, scale * 0.92);
  }
