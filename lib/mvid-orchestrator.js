...
import { resolveComfyUrl, gpuStatus, comfyAuthHeaders } from "./gpu-backend.js";
import {
  PauseError,
  StopError,
  killProcessTree,
...
export class MvidOrchestrator extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.comfyUrl = opts.comfyUrl || resolveComfyUrl() || DEFAULT_COMFY_URL;
  ...
  async interruptComfy() {
    try {
      await fetch(`${this.comfyUrl}/interrupt`, {
        method: "POST",
        headers: {
          ...comfyAuthHeaders(this.comfyUrl),
  ...
  resume() {
    ...
    this.setStage(from || "idle", `Resumed from ${from || "idle"}`);
    this.emit("sse", { type: "log", message: "▶ Pipeline resumed" });
    return { ok: true, paused: false, from };
  }
  ...
  async _spawnNode(scriptRel, args, { watchPreview = false } = {}) {
    if (this._stopped) throw new StopError();

    const script = join(ROOT, scriptRel);
    const withComfy =
      args.includes("--comfy") || !this.comfyUrl
        ? args
        : ["--comfy", this.comfyUrl, ...args];
    console.log(`\n▶ node ${scriptRel} ${withComfy.join(" ")}`);
    this.emit("sse", {
      type: "log",
      message: `▶ ${scriptRel} ${withComfy.join(" ")}`,
    });
  ...
  async _run() {
    this.comfyUrl = resolveComfyUrl(this.comfyUrl);
    this.setStage("idle", `Ensuring ComfyUI (${this.comfyUrl})…`);
